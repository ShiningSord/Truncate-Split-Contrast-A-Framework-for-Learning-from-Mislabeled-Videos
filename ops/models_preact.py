...
class TSN(nn.Module):
    ...
    def _prepare_tsn(self, num_class):
        ...
        self.project_fc =  nn.Linear(1024, 128)
    ...
    def get_optim_policies(self):
        ...
        for m in self.modules():
            ...
            elif isinstance(m, torch.nn.BatchNorm2d) or isinstance(m, torch.nn.modules.batchnorm.SyncBatchNorm):
                bn_cnt += 1
    ...
    def forward(self, input, no_reshape=False, stop_grad = False):
        if not no_reshape:
            ...
            base_out,l3x = self.base_model(input.reshape((-1, sample_len) + input.size()[-2:]))
        else:
            base_out,l3x = self.base_model(input)

        if self.dropout > 0:
            feature = base_out.clone().detach() #only for debug 
            low_dim_feature = self.project_fc(l3x)
            ...
            if stop_grad:
                base_out = self.new_fc(base_out.detach())
            else:
