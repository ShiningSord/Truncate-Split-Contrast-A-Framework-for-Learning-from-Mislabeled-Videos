...
class TSNDataSet(data.Dataset):
    ...
    def _sample_indices(self, record):
        ...
        if self.dense_sample:  # i3d dense sample
            ...
            start_index_1= 0 if sample_pos == 1 else np.random.randint(0, sample_pos - 1)
            offsets_1 = [(idx * t_stride + start_index_1) % record.num_frames for idx in range(self.num_segments)]

            start_index_2= 0 if sample_pos == 1 else np.random.randint(0, sample_pos - 1)
            offsets_2 = [(idx * t_stride + start_index_2) % record.num_frames for idx in range(self.num_segments)]
            ...
            if average_duration > 0:
                offsets1 = np.multiply(list(range(self.num_segments)), average_duration) + randint(average_duration,
                                                                                                  size=self.num_segments)
                offsets2 = np.multiply(list(range(self.num_segments)), average_duration) + randint(average_duration,
                                                                                                  size=self.num_segments)                                                                                  
            elif record.num_frames > self.num_segments:
                offsets1 = np.sort(randint(record.num_frames - self.new_length + 1, size=self.num_segments))
                offsets2 = np.sort(randint(record.num_frames - self.new_length + 1, size=self.num_segments))
            else:
                offsets1 = np.zeros((self.num_segments,))
                offsets2 = np.zeros((self.num_segments,))
            offsets = np.append(offsets1,offsets2)
            return offsets + 1
    ...
    def get(self, record, indices, index):
        ...
        try:
            prob =  self.prob_list[index]
        except:
            prob = 1
        return process_data, record.label, record.groundtruth,index, prob

    def __len__(self):
        return len(self.video_list)

    def adjust_split(self, new_split, prob):
