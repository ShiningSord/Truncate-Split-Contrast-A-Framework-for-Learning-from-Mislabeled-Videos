...
class TSN(nn.Module):
    ...
    def _prepare_base_model(self, base_model):
        ...
        if 'resnet' in base_model:
            self.base_model = getattr(torchvision.models, base_model)(False) # True if self.pretrain == 'imagenet' else 
    ...
    def adjust_segments(self, seg):
        ...
        def adjust_one_layer(stage, seg):

            blocks = list(stage.children())
            print('=>Updating stage with {} blocks residual'.format(len(blocks)))
            for i, b in enumerate(blocks):
            
                blocks[i].conv1.n_segment = seg


        self.num_segments = seg
        adjust_one_layer(self.base_model.layer1,seg)
        adjust_one_layer(self.base_model.layer2,seg)
        adjust_one_layer(self.base_model.layer3,seg)
        adjust_one_layer(self.base_model.layer4,seg)
